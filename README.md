...
将 Claude 和 ChatGPT 网页对话**按轮筛选、保持结构，并精确插入 Notion 指定位置**的 Chrome / Edge 浏览器扩展。

导出前可以预览 Notion 页面结构，选择需要保存的问答，并决定逐轮创建子页面或合并为一个页面。遇到 ChatGPT 长对话时，扩展会扫描当前活动分支；无法确认内容完整时会停止并提示，避免把残缺结果当成成功导出。
...
| 用户需求 | 本项目的处理方式 |
| --- | --- |
| 只保存真正需要的内容 | 自由勾选问答，默认不全选 |
| 放到 Notion 的准确位置 | 预览目标页面，插入到开头、末尾或指定内容块之后 |
| 控制导出后的页面结构 | 每轮问答分别创建子页面，或将多轮问答合并为一个页面 |
| 避免长对话静默缺失 | ChatGPT 自动滚动扫描当前活动分支；无法确认完整性时明确报错 |
| 尽量保留原对话格式 | 转换为 Notion 原生段落、标题、列表、代码、表格、引用和公式等 blocks |
| 让数据路径保持简单 | 浏览器直接请求 Notion API，不经过项目开发者的服务器 |
