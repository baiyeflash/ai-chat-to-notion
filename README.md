...
这是一个帮你把 Claude / ChatGPT 里的有用对话，整理进 Notion 现有笔记的 Chrome / Edge 浏览器扩展。你可以决定保存哪几轮问答、放进哪个 Notion 页面，以及接在页面的哪一段后面。

## 一个使用场景

比如，你正在和 ChatGPT 讨论“什么是递归”，又继续追问了代码示例和常见错误。问完以后，你想把其中真正有用的几轮问答，收进 Notion 的《编程笔记》，并放在“递归”这一小节后面。

这时可以点击对话页面左下角的 **N** 按钮：

1. 勾选想保存的几轮问答。
2. 选择 Notion 中的《编程笔记》。
3. 在右侧预览中找到“递归”小节，选择插入到它后面。
4. 选择把这些问答分别保存成 Notion 子页面，或者合并成一个页面，然后点击“导出到 Notion”。

扩展会把文字、标题、列表、代码和公式等内容转换成对应的 Notion 格式，并放到你刚才指定的位置。
...
| 你遇到的情况 | 它怎么帮你 |
| --- | --- |
| 一段对话里只有几轮值得保存 | 让你自己勾选，打开时不会擅自全部选中 |
| Notion 里已经有一篇整理到一半的笔记 | 显示页面内容，让你把新对话接在某个小节或段落后面 |
| 有时想分开整理，有时想放在一起 | 可以每轮问答单独建一个子页面，也可以把多轮合成一个页面 |
| ChatGPT 对话很长，担心少保存一截 | 自动滚动检查当前对话；无法确认内容完整时会告诉你，不会直接当成成功 |
| 回答里有代码、表格或公式 | 转换成 Notion 对应的格式，减少手工重新排版 |
| 不想让聊天内容经过陌生服务器 | 内容从浏览器直接发送给 Notion，不经过本项目开发者的服务器 |
...
### 怎么把 ChatGPT 对话保存到 Notion 指定位置？

打开 ChatGPT 对话，点击左下角的 **N** 按钮并勾选需要保存的问答。选择 Notion 页面后，可以把内容放在页面开头、末尾或某个小节后面。

### 能不能只保存部分 ChatGPT 或 Claude 对话？

可以。扩展打开时不会默认全选，你可以只勾选真正需要保存的几轮问答。

### 可以把多轮问答合并成一个 Notion 页面吗？

可以。你可以让每轮问答分别成为一个 Notion 子页面，也可以把选中的多轮问答合并到一个页面。

### ChatGPT 对话很长，会不会漏掉内容？

扩展会滚动扫描当前活动的对话分支，并在完成后恢复原来的滚动位置。如果无法确认已经扫描到末尾，会明确报错，不会把不完整的内容当成成功导出。
